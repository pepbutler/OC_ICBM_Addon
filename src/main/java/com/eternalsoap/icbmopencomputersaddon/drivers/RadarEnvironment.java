package com.eternalsoap.icbmopencomputersaddon.drivers;
 ...
 public class RadarEnvironment extends FrequencyEnvironment<TileRadarStation> {
  ...
  @Callback(
      doc =
          "function(s:string):boolean -- Method for finding this component when looping through
 the"
              + " component list, returns true iff s. == \"component_radar_station\"")
  ...
  @Callback(
      doc =
          "function(number) -- Set the Alarm Range of the radar. Number must be in range of [1,"
              + TileRadarStation.MAX_DETECTION_RANGE
              + "]")
  public Object[] setAlarmRange(final Context context, final Arguments args) {
    ...
    if (newRange > TileRadarStation.MAX_DETECTION_RANGE)
      throw new IllegalArgumentException(
          "Number must be in range of [1," + TileRadarStation.MAX_DETECTION_RANGE + "]");
  ...
  @Callback(
      doc =
          "function(number):boolean -- Set the Safety Range of the Radar. Number must be in range"
              + " of [1,"
              + TileRadarStation.MAX_DETECTION_RANGE
              + "]")
