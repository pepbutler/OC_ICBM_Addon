...
public class LauncherEnvironment extends ManagedTileEntityEnvironment<TileLauncherBase> {
  ...
  @Callback(
      doc =
          "function(x: number,z: number):boolean -- Launch missile towards the x and z coordinates,"
              + " if no coordinates are specified use the coordinates set by setTargetPos")
  public Object[] launchMissile(final Context context, final Arguments args) {
    BasicTargetData target = new BasicTargetData(args.checkInteger(0), 0, args.checkInteger(1));
    LauncherSolution solution = new LauncherSolution(target);
    IActionStatus status = tileEntity.missileLauncher.launch(solution, null, false);
    return new Object[] {!status.isError()};
  }

  @Callback(
      doc =
          "function(x: number,y: number, z: number):boolean -- Launch missile towards the x, y and"
              + " z coordinates, if no coordinates are specified use the coordinates set by"
              + " setTargetPos")
  public Object[] launchMissileHeight(final Context context, final Arguments args) {
    BasicTargetData target =
        new BasicTargetData(args.checkInteger(0), args.checkInteger(1), args.checkInteger(2));
    LauncherSolution solution = new LauncherSolution(target);
    IActionStatus status = tileEntity.missileLauncher.launch(solution, null, false);
    return new Object[] {!status.isError()};
  }

  // @Callback(doc = "function(s:string):boolean -- Method for finding this component when looping
  // through the component list, returns true iff s == \"component_missile_launcher\"")
  ...
  // @Callback(doc = "function(x: number, z: number) -- Set the X & Z coordinate of the target
  // position")
  // public Object[] setTargetPos(final Context context, final Arguments args) {
  //    tileEntity.launchScreen.setTarget(new Pos(args.checkInteger(0),
  // tileEntity.launchScreen.getTarget().yi(), args.checkInteger(1)));
  //    return null;
  // }

  // @Callback(doc = "function(y: number) -- Sets the detonation height for tier 2 & tier 3
  // launchers, will not work on tier 1 launchers")
  // public Object[] setDetonationHeight(final Context context, final Arguments args) {
  //    if (this.tileEntity.getTier() == EnumTier.ONE) {
  //        throw new IllegalArgumentException("Setting the Detonation-Height is not supported on
  // launchers of tier 1");
  //    }
  //    tileEntity.launchScreen.setTarget(new Pos(tileEntity.launchScreen.getTarget().xi(),
  // args.checkInteger(0), tileEntity.launchScreen.getTarget().zi()));
  //    return null;
  // }

  // @Callback(doc = "function(): number -- Gets the detonation height for tier 2 & tier 3
  // launchers, will not work on tier 1 launchers")
  // public Object[] getDetonationHeight(final Context context, final Arguments args) {
  //    if (this.tileEntity.getTier() == EnumTier.ONE) {
  //        throw new IllegalArgumentException("Getting the Detonation-Height is not supported on
  // launchers of tier 1");
  //    }
  ...
  // */
  // @Callback(doc = "function(x: number,z: number):boolean -- Launch missile at stored position or
  // by providing the x and z coordinates")
  // public Object[] launch(final Context context, final Arguments args) {
  //    return launchMissile(context, args);
  // }

  // @Callback(doc = "function(x: number,z: number):boolean -- Launch missile towards the x and z
  // coordinates, if no coordinates are specified use the coordinates set by setTargetPos")
  // public Object[] launchMissile(final Context context, final Arguments args) {
  ...
  // @Callback(doc = "function(x: number,z: number):boolean -- Get whether the given position is
  // within range.")
  ...
  // @Callback(doc = "function():number -- Get the Frequency of the launcher, only works when the
  // tier supports the frequency")
  ...
  // {
  //        throw new IllegalArgumentException("Getting the frequency is not supported on launchers
  // of tier 1 and tier 2");
  //    }
  //    return new Object[]{tileEntity.launchScreen.getFrequency()};
  // }

  // @Callback(doc = "function(number):boolean -- Set the Frequency of the launcher, only works when
  // the tier supports the frequency")
  // public Object[] setFrequency(final Context context, final Arguments args) {
  //    if (this.tileEntity.getTier() == EnumTier.ONE || this.tileEntity.getTier() == EnumTier.TWO)
  // {
  //        throw new IllegalArgumentException("Setting the frequency is not supported on launchers
  // of tier 1 and tier 2");
  //    }
  ...
  // @Callback(doc = "function():number -- Get the LockHeight of the launcher, only works when the
  // tier supports the LockHeight")
  // public Object[] getLockHeight(final Context context, final Arguments args) {
  //    if (this.tileEntity.getTier() == EnumTier.ONE) {
  //        throw new IllegalArgumentException("Getting the Lock-Height is not supported on
  // launchers of tier 1");
  //    }
  //    return new Object[]{tileEntity.launchScreen.lockHeight};
  // }

  // @Callback(doc = "function(number):boolean -- Set the LockHeight of the launcher, only works
  // when the tier supports the LockHeight")
  // public Object[] setLockHeight(final Context context, final Arguments args) {
  //    if (this.tileEntity.getTier() == EnumTier.ONE) {
  //        throw new IllegalArgumentException("Setting the Lock-Height is not supported on
  // launchers of tier 1");
  //    }
